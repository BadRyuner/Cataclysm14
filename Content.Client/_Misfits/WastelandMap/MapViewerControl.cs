...
public sealed class MapViewerControl : Control
{
	...
	private WastelandMapTrackedBlip[] _trackedBlips = Array.Empty<WastelandMapTrackedBlip>();
	private WastelandMapAnnotation[] _annotations = Array.Empty<WastelandMapAnnotation>();
    ...
    public void SetTrackedBlips(WastelandMapTrackedBlip[] trackedBlips) => _trackedBlips = trackedBlips;
    public void SetAnnotations(WastelandMapAnnotation[] annotations) => _annotations = annotations;
    ...
    protected override void Draw(DrawingHandleScreen handle)
    {
        ...
        if (TryGetPlayerUv(out var playerUv))
        {
            ...
            DrawPlayerMarker(handle, markerPos);
        }
        ...
        {
            DrawAnnotation(handle, new WastelandMapAnnotation(
    ...
    // grid coordinates matches "self pos"
    if (TryGetUv(xform.Coordinates.Position, out uv))
        return true;
    ...
    return TryGetUv(xformSystem.GetMapCoordinates(xform).Position, out uv);
	}
    ...
    protected override void KeyBindDown(GUIBoundKeyEventArgs args)
    {
        ...
        if (_annotationMode == AnnotationMode.Marker)
        {
            ...
            {
                OnAddAnnotation?.Invoke(new WastelandMapAnnotation(WastelandMapAnnotationType.Marker, uv.X, uv.Y, uv.X, uv.Y, GetAnnotationLabel("Marker"), ToPackedColor(_currentColor), _currentStrokeWidth, null));
                args.Handle();
            }
            return;
        }
    ...
    protected override void KeyBindUp(GUIBoundKeyEventArgs args)
    {
        ...
        if (_annotationMode == AnnotationMode.Draw && _freeDrawActive)
        {
            ...
            if (_freeDrawUvPoints.Count >= 2)
            {
                ...
                OnAddAnnotation?.Invoke(new WastelandMapAnnotation(WastelandMapAnnotationType.Draw, 0f, 0f, 0f, 0f, GetAnnotationLabel("Drawing"), ToPackedColor(_currentColor), _currentStrokeWidth, pts));
            }
            _freeDrawUvPoints.Clear();
            args.Handle();
            return;
        ...
        if (_annotationMode == AnnotationMode.Box && _annotationDragStartUv.HasValue && _annotationDragCurrentUv.HasValue)
        {
            OnAddAnnotation?.Invoke(new WastelandMapAnnotation(WastelandMapAnnotationType.Box, _annotationDragStartUv.Value.X, _annotationDragStartUv.Value.Y, _annotationDragCurrentUv.Value.X, _annotationDragCurrentUv.Value.Y, GetAnnotationLabel("Box"), ToPackedColor(_currentColor), _currentStrokeWidth, null));
            _annotationDragStartUv = null;
            _annotationDragCurrentUv = null;
            args.Handle();
            return;
    ...
    private void DrawPlayerMarker(DrawingHandleScreen handle, Vector2 pos)
    {
        var pulse = (float) (0.5 + 0.5 * Math.Sin(_gameTiming.RealTime.TotalSeconds * 4.0));
        handle.DrawCircle(pos, 24f + 10f * pulse, new Color(1f, 0.2f, 0.2f, 0.35f * pulse));
        handle.DrawCircle(pos, 18f, new Color(0f, 0f, 0f, 0.55f));
        handle.DrawCircle(pos, 14f, new Color(1f, 0.15f, 0.15f, 1f));
        handle.DrawCircle(pos, 6f, Color.White);
    }

    private static void DrawBlip(DrawingHandleScreen handle, Vector2 pos, WastelandMapTrackedBlipKind kind)
    {
    ...
    private void DrawAnnotation(DrawingHandleScreen handle, WastelandMapAnnotation annotation, float x, float y, float drawW, float drawH, bool preview = false, Color? overrideColor = null)
    {
    ...
    private static Color GetTrackedBlipColor(WastelandMapTrackedBlipKind kind)
    {
        switch (kind)
        {
            ...
            case WastelandMapTrackedBlipKind.PipBoyContact:
                return new Color(0.2f, 0.85f, 1f, 1f);
            case WastelandMapTrackedBlipKind.PipBoyGroupMember:
